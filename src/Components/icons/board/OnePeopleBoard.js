...
const OnePeopleBoard = (details) => {
  const [invisible, setInvisible] = React.useState(false);

  const dis = useDispatch();
  const userSelect = useSelector((state) => state.user.selectedUser); // בעל הדירות ומשתמש נוכחי
  let ownerid = details.props.Id; //בעל הדירה שאיתו אני מתכתבת

  function savearrchat() {
    getRoomFromServer(ownerid,userSelect.Id)
      .then((res) => {
  ...
  return (
    <>
      ...
      >
        ...
        <ListItemText
          primary={details.props.Name}
          secondary={details.props.Mail}
        />
      </ListItemButton>
    </>
  );
};
export default OnePeopleBoard;
