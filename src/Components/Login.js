...
const schema = yup
  .object({
    password: yup
      .string()
      .required("שדה חובה")
      .test("len", "אורך לא תקין", (x) => x.length <= 10 && x.length >= 4),
    email: yup.string().email("כתובת מייל שגויה"),
  })
  .required();
...
export default function Login() {
  ...
  const propsUrl = useSelector((state) => state.prop.propsUrl);
  ...
  const login = (e) => {
    ...
    getLogin(details)
      .then((res) => {
        ...
        if (!res.data.user)
          //כאשר אין משתמש כזה יציג הודעת שגיאה
          alert(res.data.mass);
        else {
          if (res.data.user.Active.data[0] == 1) {
            dis(SaveUser(res.data.user));
            console.log("propsUrl in loginnnn");
          ...
          } else alert("אופס... משתמש זה חסום אנא פנה למנהל האתר");
        }
      })
      .catch((err) => {
        alert(err.mass);
